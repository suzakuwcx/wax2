	...
	** settings at this point. */
	...
	while (1)
	{
		...
		if (n > 0 && FD_ISSET(s, &readfds))
		{
			...
			/* check the output string format to judge if success or failed */
			if (is_boot) 
			{
				if (dfa_check(success, buf, len) == 0)
				{
					printf("\r\n[Success]\r\n");
					return 3;
				}

				if (dfa_check(invalid_token_dfa, buf, len) == 0)
				{
					printf("\r\n[Invalid token]\r\n");
					return 4;
				}
...
int
push_main(const char *command)
{
	struct packet pkt;
	int ret;
	int s;
	FILE *fp = fmemopen((char *)command, strlen(command), "r");
	...
	for (;;) {
		/* Push the contents of command to the socket. */
		pkt.type = MSG_PUSH;

		ssize_t len;

		len = fread(pkt.u.buf, 1, sizeof(pkt.u.buf), fp);
